...
/// 2. Create additional windows for remaining tabs
/// 3. Send startup commands to tabs that have them
/// 4. Set @muster_* metadata
pub fn create_from_profile(client: &TmuxClient, profile: &Profile) -> Result<SessionInfo> {
    ...
    client.new_session(&session_name, &first_tab.name, &first_tab.cwd)?;
    ...
    for (i, tab) in profile.tabs.iter().enumerate().skip(1) {
        client.new_window(&session_name, &tab.name, &tab.cwd)?;
...
mod tests {
    ...
    fn test_create_session_from_profile() {
        ...
        let info = create_from_profile(&client, &profile).expect("create session");
    ...
    fn test_create_session_sets_metadata() {
        ...
        create_from_profile(&client, &profile).expect("create session");
